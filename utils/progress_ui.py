...
class ProgressTracker:
    ...
    def _build_html(self) -> str:
        ...
        return (
            '<div class="prg-panel">'
            "<style>"
            ".prg-panel{background:#FFFFFF;border:1px solid rgba(0,0,0,0.08);"
            ...
            ".prg-dot{width:8px;height:8px;border-radius:50%;background:#A0A0A5;display:inline-block}"
            ".prg-dot-active{background:#34C759;animation:prg-pulse 1.5s ease infinite}"
            ".prg-dot-done{background:#34C759}"
            ".prg-label{font-size:0.875rem;color:#1C1C1E;font-weight:500}"
            ".prg-counter{font-size:1.75rem;font-weight:700;color:#1C1C1E;letter-spacing:-0.02em;line-height:1}"
            ".prg-bar{height:3px;background:#E5E3DE;border-radius:2px;overflow:hidden}"
            ".prg-bar-fill{height:100%;border-radius:2px;transition:width 0.6s cubic-bezier(0.4,0,0.2,1)}"
            ".prg-bar-active{background:linear-gradient(90deg,#1C1C1E,rgba(28,28,30,0.4));"
