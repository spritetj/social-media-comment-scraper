...
class OneSearchProgress:
    ...
    def on_message(self, msg: str):
        ...
        # Skip internal/technical messages — don't show to user
        _skip_prefixes = ("Research: ", "Hypotheses: ", "Query:", "Brand: ", "Intent: ")
        ...
        msg_lower = msg.lower()
        if "searching" in msg_lower or "preparing" in msg_lower:
            self.current_step = 0
        elif "found" in msg_lower and ("content" in msg_lower or "url" in msg_lower):
            self.current_step = 1
        elif "scrap" in msg_lower or "collecting" in msg_lower:
            self.current_step = 2
        elif "analyz" in msg_lower or "analysis" in msg_lower:
            self.current_step = 3
        elif "insight" in msg_lower or "report" in msg_lower:
            self.current_step = 4
