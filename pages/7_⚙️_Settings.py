...
==============================
Configure API keys for paid AI providers (Claude, ChatGPT, Gemini).
Keys are stored only in session state, never written to disk.

Note: Search API and NotebookLM configuration are in the One Search page.
"""
...
st.markdown(
    '<p class="page-desc">Configure API keys for paid AI providers. '
    "Search API and NotebookLM are configured directly in One Search.</p>",
...
for provider_slug, info in PROVIDERS.items():
    with st.expander(f"{info['name']}", expanded=(provider_slug == get_active_provider())):
        st.markdown(
